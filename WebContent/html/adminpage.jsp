        		...
        		<div style="margin: 30px;">
              	...
              	<div>
					...
					<%
						...
						out.println("<form action='http://localhost:5000/getdetails/" +  channelname  + "'>");
						out.println("<button class='btn btn-outline-primary my-2 my-sm-0'  >Survey Details</button>");
						out.println("</form>");
					%>
					...
					<p></p>
					<form action='audittrail'  method='POST'>
						<input type="hidden" value="<%= channelname1 %>" name="channelname">
						<button class='btn btn-outline-primary my-2 my-sm-0'  >Audit Trial</button>
					</form>
      ...
      /*
    	...
    	function notestrtojson(item, index){
      	...
      	function reviewfunction(item, index) {
        	...
        	var newpele = document.createElement("p");
        	newpele.innerHTML = date + " " + time;
			var markasdone = document.createElement("button");
			var markasreject = document.createElement("button");
        	
        	// mark as completed or reject
        	markasdone.onclick = markdonefunc;
        	markasdone.innerHTML = "Mark as Done";
        	
        	markasreject.onclick = markrejectfunc ;
        	markasreject.innerHTML = "Mark as Reject";
	    	...
	    	divele.append(markasdone);
	    	divele.append(markasreject);
      	...
      	function markdonefunc(){
	    	...
	    	var taskid = hidinp.value;
      		...
      		statusajaxfunc(taskid, status)
      	}
      	function markrejectfunc(){
      		var ele = this.parentNode;
      		var hidinp = ele.querySelector("#hidinp");
	    	console.log(hidinp);
	    	var taskid = hidinp.value;
      	...
      	function statusajaxfunc(taskid, status){
      		var xhr = new XMLHttpRequest();
			
			var data = "taskid=" + encodeURIComponent(taskid)
	        + "&status=" + encodeURIComponent(status);
			xhr.open("POST", '/agilemeetingdash/changestatus', true);
      	...
      	function ajaxfunc(noteid, newtitle){
			...
			xhr.open("POST", '/agilemeetingdash/editnotes', true);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xhr.onload = () =>{
      	...
      	function ajaxassignfunc(noteid, channelname , newdate, memberemail){
			...
			xhr.open("POST", '/agilemeetingdash/assigntask', true);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xhr.onload = () =>{
